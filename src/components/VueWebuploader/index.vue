...
export default {
  ...
  watch: {},
  mounted() {
    this.$store.dispatch('fileUpload/isVisibility', 2)
  },
  created() {},
  methods: {
    // 关闭
    ...
    // 最小化
    minimality() {
      this.$store.dispatch('fileUpload/isVisibility', 3)
    },
    // 展开
    open() {
      this.$store.dispatch('fileUpload/isVisibility', 1)
    },
    ...
    onSuccess(file, response) {
      ...
      if (currentFile.data_type === 3) {
        ...
        deskAddFile(params).then(res => {
          ...
          this.$store.dispatch('fileUpload/deskAddFileSuccess', res.data)
        })
      } else {
        ...
        knowledgeCreateFile(params).then(res => {
          // console.log(res)
          this.$message.success('上传成功！')
          this.$store.dispatch('fileUpload/createFileSuccess', res.data)
        })
      }
    },
...
.file-item .progress ::v-deep .el-progress-bar {
  width: 95%;
}

#filePicker ::v-deep .webuploader-pick {
  width: 100% !important;
  height: 100% !important;
  background-color: #fafafa !important;
}

#filePicker ::v-deep .webuploader-pick-hover {
  background-color: #fafafa !important;
}
